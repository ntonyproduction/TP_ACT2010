...
\newline\newline
On peut alors faire l'analyse de nos résidus pour faire notre choix parmi nos trois modèles. Les résidus de ces trois modèles sont tracés à la \textit{Figure 8}.
...
\begin{tabular}{| l c | r |}
  ...
  ARI(1,1) & $\mu$ & 0\\
    & $\phi$ & $`r round(ttaux011$coef,4)`$\\ \hline
  IMA(1,1) & $\mu$ & 0\\
    & $\theta$ & $`r round(ttaux110$coef,4)`$\\ \hline
  ARIMA(1,1,1) & $\mu$ & 0\\
...
abline(h=0)
```
\textbf{Figure 8.} Superposition du graphique 
