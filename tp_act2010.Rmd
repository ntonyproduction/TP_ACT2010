...
## e)
On cherche maintenant le modèle de notre série stationnaire. En observant le graphique ACF et PACF de la figure 5 et 6, 

```{r, eval=T, echo=F, fig.height=3.3}
acf(diff(log(ttaux)), main="Figure 5: fonction d'autocorrélation")
pacf(diff(log(ttaux)), main="Figure 6: fonction d'autocorrélation partielle")
```

il semblerait que l'on soit en présence d'un AR(1) pour la partie autorégressive. Pour ce qui est de la partie moyenne mobile, il semblerait que l'on ait une MA(1) puisque que **le graphique EACF nous dit sa, mais faudrais dire kkchose a propos du graphique acf fak jsais potro**... Nous confirmons le tout avec la fonction d'autocorrélation étendue (EACF) ci-dessous.

```{r, eval=T, echo=F, fig.height=3.3}
eacf(diff(log(ttaux)))
```

Ce graphique porte à croire que l'on est en présence d'un modèle ARMA(0,1), i.e. MA(1), ou un **ARMA(1,1) PAS PLUTÔT ARMA(2,1)???** pour la première différence du logarithme.
